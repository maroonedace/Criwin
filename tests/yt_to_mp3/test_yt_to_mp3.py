...
class TestYtToMp3:
    ...
    async def test_multiple_downloads(self, mock_interaction):
        ...
        with patch('src.commands.yt_to_mp3.setup.send_message', new_callable=AsyncMock) as mock_send_message:
    ...
    async def test_invalid_url_structure(self, mock_interaction):
        ...
        with patch('src.commands.yt_to_mp3.setup.send_message') as mock_send_message:
            # Run the async function
            await setup_yt_to_mp3(mock_interaction, url)
    ...
    async def test_download_clip(self, mock_interaction):
        ...
        with patch('src.commands.yt_to_mp3.setup.send_message') as mock_send_message:
            with patch('src.commands.yt_to_mp3.setup.download_clip') as mock_download_clip:
